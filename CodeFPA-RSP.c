...
double rev (double x, double y)
{
	...
	//3 - Becker and Lago Problem x = (-5,5), (5,-5), (-5,-5), (5,5) e f(x) = 0 [-10,10]
	//resp = pow((fabs(x) - 5),2) + pow((fabs(y) - 5),2);						 
	
	//5 - Bohachevsky 2 Problem x = (0,0) e f(x) = 0 [-50,50]
	//resp = pow(x,2) + 2*(pow(y,2)) - 0.3*(cos(3*M_PI*x))*(cos(4*M_PI*y)) + 0.3;	
	
    //6 - Branin Problem x = (-M_PI , 12.275), (M_PI , 2.275), (9.42478, 2.475) e f(x) = 0.397887        
	//resp = pow((y - (b*(x*x)) + (c*x) - 6),2) + 10*(1-h)*cos(x) + 10;      
	
    //7 - Camel Back–3 Three Hump Problem x = (0,0) e f(x) = 0 [-5,5]
	//resp = 2*(pow(x,2)) - 1.05*(pow(x,4)) + (1.0/6)*(pow(x,6)) + x*y + pow(y,2);     		
	
    //11 - Easom Function x = (M_PI, M_PI) e f(x) = -1 [-10,10]
	//resp = -cos(x)*cos(y)*exp(-((x-M_PI)*(x-M_PI)) - (y-M_PI)*(y-M_PI));      													

	//14 - Goldstein and Price x = (0,-1) e f(x) = 3 [-2,2]
	//resp = (1+(pow((x+y+1),2))*(19-14*x+3*(pow(x,2))-14*y+6*x*y+3*(pow(y,2))))*(30+(pow((2*x-3*y),2))*(18-32*x+12*(pow(x,2))+48*y-36*x*y+27*(pow(y,2))));	

	//28 - Modified Rosenbrock Problem x = (0.3412,0.1164), (1,1) e f(x) = 0 [-5,5]
    //resp = 100*(pow(y-(pow(x,2)),2)) + (pow(6.4*(pow(y-0.5,2))-x-0.6,2));                     						

	//34 - Periodic Problem x = (0,0) e f(x) = 0.9 [-10,10]
    //resp = 1 + (pow(sin(x),2)) + (pow(sin(y),2)) - 0.1*(exp(-(pow(x,2))-(pow(y,2))));  

	//40 - Schaffer 1 Problem x = (0,0) e f(x) = 0 [-100,100]
    //resp = 0.5 + ((pow(sin(sqrt((pow(x,2))+(pow(y,2)))),2))-0.5)/(pow(1+0.001*((pow(x,2))+(pow(y,2))),2));  

	//41 - Schaffer 2 Problem x = (0,0) e f(x) = 0 [-100,100]
    //resp = (pow((pow(x,2))+(pow(y,2)),0.25))*((pow(sin(50*(pow((pow(x,2))+(pow(y,2)),0.1))),2))+1);  
	  
	
	//4. Himmelblau problem 9 raizes f(x) = 0 [-5,5] 
